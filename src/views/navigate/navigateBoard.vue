        ...
        <n-tab-pane name="chart" tab="數據圖">
          <n-space class="mbr_2" align="center">
            ...
            :shortcuts="rangeShortcuts"
             />
            <n-button type="info"  @click="getRangeVisitors(navigateChartSearch.dailyRangeVisitorsFormat ?? millisecondsToSeconds(navigateChartSearch.dailyRangeVisitors))">查詢</n-button>
          </n-space>
          ...
          <n-space class="mbr_2" align="center">
            日期(日) 
            <n-date-picker  v-model:value="navigateChartSearch.dailyLiveUsers" v-model:formatted-value="navigateChartSearch.dailyLiveUsersFormat" :actions="null" type="date" value-format="t" placeholder=""   />
            <n-button type="info" @click="getDailyLiveUsers(navigateChartSearch.dailyLiveUsersFormat ?? Math.floor(navigateChartSearch.dailyLiveUsers / 1000))">查詢</n-button>
          </n-space>
          ...
          <n-space class="mbr_4" align="center">
            訪問開始時間起迄（日/時）
            <n-date-picker v-model:value="navigateChartSearch.dateRangeAccess" 
            ...
            value-format="t"
            :actions="null"
            :shortcuts="rangeShortcuts" update-value-on-close />
            ...
            <n-button type="info" @click="getRangeAccess(navigateChartSearch.dateRangeAccessFormat ?? millisecondsToSeconds(navigateChartSearch.dateRangeAccess))">查詢</n-button>
          </n-space>
          <div class="chart_pie ">
...
import type { DailyData, DailyReq, VisitData, DateRange, PieData, PagerReq } from '@/models/api/navigate';

import { apiGetAnalysisData, apiGetDailyData, apiGetDateVisitData,  apiListPager } from '@/apis/navigate';

import { DeviceOptions, UserTypeOptions, VisitItemOptions, VisitTypeOptions } from '@/utils/dropdownOptions';
...
const getPreMonthFirstDay = dayjs().year(year).month(month - 1).startOf('month').valueOf();
const getPreMonthLastDay = dayjs().year(year).month(month - 1).endOf('month').valueOf();
...
const getList = () => {
  const query: PagerReq = {
    start_time: searchData.value.dateRange ? Math.floor(searchData.value.dateRange[0] / 1000) : undefined,
    end_time: searchData.value.dateRange ? Math.floor(searchData.value.dateRange[1] / 1000) : undefined,
...
onMounted(() => {
  const defaultVisitDate = navigateChartSearch.value.dailyRangeVisitors.map((date) => Math.floor(date / 1000));
